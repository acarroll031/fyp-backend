...
from data_processing import convert_grades_to_students
...
oauth2_scheme = OAuth2PasswordBearer(
    tokenUrl="token"
)  # Setup OAuth2 for token handling
...
@app.post(
    ...
    description="Predict the risk score of a student based on their performance features.",
)
def predict_risk(request: PredictRequest, model=Depends(get_model)):
    """
...
@app.get(
    "/students",
    summary="Get Students for Lecturer",
    description="Retrieve a list of students associated with the logged-in lecturer.",
)
def get_students(
...
@app.get(
    "/students/{student_id}/{module_id}",
    summary="Get Student Details by Module",
    description="Retrieve detailed information about a specific student for a specific module, including grades and risk history.",
)
def get_student_details_by_module(
    student_id: str,
    module_id: str,
    current_user_email: str = Depends(get_current_user),
    connection=Depends(get_db_connection),
...
@app.post(
    "/students/{module_id}/grades",
    summary="Upload Grades and Update Risk Scores",
    description="Upload a CSV file containing student grades for a specific module, update the grades in the database, and recalculate risk scores using the ML model.",
)
async def post_grades(
    ...
    try:
        ...
        total_grades_df = pd.read_sql_query(
            "SELECT * FROM grades WHERE module = %s", connection, params=(module_id,)
        )
        ...
        existing_scores_query = (
            "SELECT student_id, risk_score FROM risk_scores WHERE module = %s"
        )
        existing_scores_df = pd.read_sql_query(
            existing_scores_query, connection, params=(module_id,)
        )

        # Rename column to avoid confusion
        existing_scores_df = existing_scores_df.rename(
            columns={"risk_score": "previous_risk_score"}
        )
        ...
        student_df = pd.merge(
            student_df, existing_scores_df, on="student_id", how="left"
        )
        ...
        cursor.execute(
            "SELECT lecturer_email FROM modules WHERE module_code = %s", (module_id,)
        )
...
@app.post(
    ...
    description="Register a new lecturer with email, password, and name.",
)
def register_lecturer(lecturer: LecturerCreate, connection=Depends(get_db_connection)):
    """
...
@app.post(
    ...
    description="Authenticate a lecturer and receive a JWT token.",
)
def login(
    ...
    cursor.execute(
        """ SELECT * FROM lecturers WHERE email = %s""", (form_data.username,)
    )
    lecturer = cursor.fetchone()
    connection.close()

    # Verify password
    if not lecturer or not verify_password(
        form_data.password, lecturer["password_hash"]
    ):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    # Create JWT token
    access_token = create_access_token(data={"sub": lecturer["email"]})
...
@app.post(
    ...
    description="Create a new module associated with the logged-in lecturer.",
)
def create_module(
    module: ModuleCreate,
    ...
    try:
        ...
        connection.rollback()
        raise HTTPException(
            status_code=400, detail="Module code already exists"
...
@app.get(
    "/modules",
    summary="Get Modules",
    description="Retrieve a list of modules associated with the logged-in lecturer.",
)
def get_modules(
    current_user_email: str = Depends(get_current_user),
    connection=Depends(get_db_connection),
...
@app.put(
    ...
    description="Update details of a specific module associated with the logged-in lecturer.",
)
def update_module(
    module_code: str,
...
@app.delete(
    ...
    description="Delete a specific module associated with the logged-in lecturer.",
)
def delete_module(
    module_code: str,
    current_user_email: str = Depends(get_current_user),
...
@app.get(
    ...
    description="Retrieve notifications for the logged-in lecturer about students at risk.",
)
def get_notifications(
    current_user: str = Depends(get_current_user), connection=Depends(get_db_connection)
):
    """
...
@app.put(
    ...
    description="Mark a specific notification as unread for the logged-in lecturer.",
)
...
@app.put(
    ...
    description="Mark a specific notification as read for the logged-in lecturer.",
)
def mark_notification_as_read(
    notification_id: int,
    current_user: str = Depends(get_current_user),
