...
def preprocess_module_data(
    csv_file, module_code, total_assessments, assessment_total_score
):
    """
    Preprocess the student lab data for a given module.
    This includes removing anonymising student ids, identifying information, reshaping the data,
    ...
    salt = csv_file  # Use a different salt for each dataset
    student_labs["Student ID"] = student_labs["Student ID"].apply(
        lambda x: anonymise_id(x, salt)
    )

    # Reshape the DataFrame from wide to long format
    student_labs = pd.melt(
        student_labs,
        id_vars=["Student ID", "TOTAL"],
    ...
    student_labs["assessment_name"] = (
        student_labs["assessment_name"].str.extract(r"(\d+)").astype(int)
    )
    student_labs = student_labs.rename(columns={"assessment_name": "assessment_number"})

    # Add a new column for progress in semester
    student_labs["progress_in_semester"] = (
        student_labs["assessment_number"] / total_assessments
    )
    ...
    new_filename = base_filename + "_processed.csv"
...
def training_data(csv_file, progress_threshold):
    ...
    ******************************************************************************
    ...
    student_labs = student_labs[
        student_labs["progress_in_semester"] <= progress_threshold
    ]

    # Group by Student ID and calculate required metrics
    student_labs = (
        student_labs.groupby("Student ID")
        .agg(
    ...
    base_filename = csv_file.split("training_data/")[1].split("processed.csv")[0]
    new_filename = (
        "training_data/"
        + base_filename
        + "training_"
        + str(progress_threshold)
        + ".csv"
    )
    student_labs.to_csv(new_filename, index=False)
...
def convert_grades_to_students(grades_df):
    ...
    students_df = (
        grades_df.groupby(["student_id", "module"])
        .agg(
